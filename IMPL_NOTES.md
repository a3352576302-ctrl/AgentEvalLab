...
DeepSeek rag_document_qa 仅 6/20 (30%) vs MiniMax 19/20 (95%)。根因：knowledge 工具只做单向 `key in query` 匹配，DeepSeek 查询措辞稍不同就失败 → 重复调用 → max_rounds 超限。

### 实现

5 层匹配策略（`tools.py`）：
1. 原始双向包含（`key in query` OR `query in key`）
2. 归一化后双向包含（去标点、全角半角、大小写）
3. 别名/同义词映射（`_KNOWLEDGE_ALIASES`）
4. 无匹配候选提示（减少 LLM 重复调用）
...
- 去重改为 `tool + normalized_args` 级别（不再按 tool_name 粗去重）
- 命中时复用首次真实 ToolResult（含原始数据），仅加 `deduped: True` 标记
- 新增 `_normalize_params`：去空格/标点/全角半角/大小写
- **395 passed + 226 skipped** 🟢（+3 dedup 测试）
...
| MT-003 温度穿搭 | ✅ | ✅ |
| MT-005 天气穿搭 | ✅ | ✅ |
| knowledge 深度查询 | ❌ | ❌（知识库无相关条目） |

### 诚实结论

去重逻辑已正确：`_normalize_params` 级去重 + 复用真实结果 + 3 条测试覆盖。但 smoke 通过率未显著变化，根因不在去重：

1. **知识库空缺**：无 "chunk策略" "RAG vs 微调" "Agent ReAct" 条目。模型查询失败后换措辞重试→去重不命中（参数不同）→循环。
2. **MT 工具选择**：模型选 weather 而非 calculator（如 MT-028）、调 weather 不调 knowledge（MT-021）——非确定性决策。
3. **部分 "失败" 是语义正确但 tool_sequence 不同**（MT-029: knowledge→calculator 也能完成任务）。

prompt + orchestration 层优化到此边界。后续提升需知识库内容扩展 + 语义等价断言。

### 下一步

由用户决定：知识库内容扩展 / LLM-as-Judge / 进入 Stage 3 安全拒答 / 简历投递
