...
## v1.1 Stage 2.2：Knowledge 内容扩展后 API 验证

**日期：** 2026-06-22

### 原来哪里差？

v1.1 Stage 2 中 GEN-KNOW-011（chunk策略）、GEN-KNOW-012（ReAct）、GEN-KNOW-014（RAG vs 微调）3 条全部失败。根因不是 prompt 不够——是知识库根本缺少这些深度条目。模型查询→返回空→换措辞重试→max_rounds 超限。3 条中有 3 条是知识库覆盖不足。

### 改了什么？

`agentevallab/tools.py`（已完成，用户操作）：
- 新增 `_KNOWLEDGE_DB` 条目：`RAG分块策略`、`什么是ReAct Agent`、`RAG和微调对比`
- 新增对应别名映射（3 组，含中英文变体）
- 别名放在泛化条目之前，避免具体问题被泛化条目抢走

`tests/test_tools.py`（已完成，用户操作）：
- 3 条新测试验证 3 个新条目命中正确
...
| 方案 | 优点 | 缺点 | 选择 |
|------|------|------|------|
| **A: 扩展静态知识库（当前）** | 零成本，即时生效，问题明确 | 每次缺内容需手工加 | ✅ 选用 |
| B: 向量检索 | 语义匹配强 | 引入 ~100MB 依赖，需向量化 DB | ❌ 太重 |
| C: LLM-as-Judge 放宽断言 | 不依赖知识库覆盖 | 每次多调一次 API，且放弃对 knowledge 工具正确性的验证 | ❌ 不改 benchmark |

当前 3 个缺失问题明确、范围小，方案 A 是最小代价。

### 数据有没有变好？

**pytest：** 399 passed + 226 skipped（+3 新知识测试）  
**API Smoke（GEN-KNOW-011/012/014，v1.0 全部失败）：**

| 用例 | v1.0 | v1.1 Stage 2.2 DS | v1.1 Stage 2.2 MM |
|------|------|-------------------|-------------------|
| GEN-KNOW-011 (chunk策略) | ❌ | ✅ PASS | ✅ PASS |
| GEN-KNOW-012 (ReAct) | ❌ | ✅ PASS | ✅ PASS |
...
## v1.1 收尾：全量回归 + 成本 + Tool Metrics

**日期：** 2026-06-22

### v1.1 vs v1.0 全量对比

| 指标 | DS v1.0 | DS v1.1 | MM v1.0 | MM v1.1 |
|------|---------|---------|---------|---------|
| 通过率 | 65.0% | **74.8%** +9.8pp | 75.2% | 74.3% -0.9pp |
| Security | 75% | **85%** +10pp | 70% | 62% -8pp* |
| P95 | 8.57s | 7.82s | 8.28s | 8.51s |

*MM security 下降可能是采样波动。

### 新增模块

- `agentevallab/cost.py`：价格配置 + 成本计算 + 月估算
- `agentevallab/tool_metrics.py`：工具成功率/重复率/失败率统计
- `reports/benchmark-v1.1-full-summary.md`：完整对比报告

### 验证

- **420 passed + 226 skipped** 🟢（+13 新测试）
- DS 169/226 (74.8%), MM 168/226 (74.3%)
- 成本：DS ~$0.13/run, MM ~$0.35/run
...
> 以下按阶段总结项目的对外能力演进，与 README 口径一致，不使用内部版本号。

### 第一阶段：评测框架核心闭环

- YAML 驱动用例管理，pytest 参数化回归
- L1-L6 多层断言引擎（结果 / 工具 / 参数 / 轮次 / 安全 / Token）
- RuleBasedAgent + LLMAgent 双 Agent 模式
- 6 种故障注入（超时 / 500 / 脏数据 / 空结果 / 权限拒绝 / 网络不可达）
- 控制台 + HTML + JUnit XML 报告，GitHub Actions 持续集成
- 安全用例体系：Prompt 注入、越权调用、敏感信息泄露、拒答检测

### 第二阶段：测评体系与规模化

- 用例 Schema 标准化（scene / difficulty / priority / golden_answer）
- 17 种失败归因自动分类（区分模型缺陷 / 断言过严 / Provider 错误 / 安全风险）
- API 重试（指数退避）+ 中断续跑（run JSON 持久化）
- HTML 报告增强：框架通过率 vs 模型任务完成率双指标、归因 Top-N、完整 tool trace
- Baseline 回归检测（通过率 / P95 / Token / 安全 四维退化判断）

### 第三阶段：平台化与 Benchmark v1.0

- 模型注册表（集中管理 provider / base_url / 价格 / 能力标签）
- 静态 Dashboard（历史运行列表 / 总览卡片 / 模型对比表 / 归因统计）
- Docker 一键复现环境
- FastAPI 服务化（任务提交 / 状态查询 / 结果查询 / 报告端点）
- SQLite 数据持久化（runs / case_results / tool_traces / review_items）
- HTTPAgent 适配器（测外部 Agent 系统）
- 339 条 YAML Benchmark（手写 + 模板生成 + 场景用例）
- 9 类生成器，226 条 LLM-only 用例，113 条 RuleBaseline

### 第四阶段：模型评测与优化闭环

- DeepSeek + MiniMax 双模型 226 条全量评测
- Knowledge 模糊匹配（5 层策略：双向包含 / 归一化 / 别名映射 / 候选提示）
- Multi-tool prompt 策略优化 + 去重守卫
- Knowledge 内容扩展（新增 chunk 策略 / ReAct / RAG vs 微调 三个深度条目）
- 语义复核（规则版）：区分"真失败"和"工具路径不同但任务完成"
- Token 成本估算 + 工具调用统计（成功率 / 重复率 / 失败率）
- v1.1 全量回归：DeepSeek 65.0% → 74.8%，rag_document_qa 6/20 → 16/20
