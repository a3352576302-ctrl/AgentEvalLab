...
P2.6 全量评测数据：DeepSeek rag_document_qa 6/20 (30%) vs MiniMax 19/20 (95%)。从 `ds-bench-v1.json` 逐条分析 14 条 DS 失败 case，全部是 tool_sequence mismatch——knowledge 工具调用后返回空（`knowledge 工具只做单向 key in query`），模型换措辞重试→失败→再重试→max_rounds 超限。
...
**仍失败 4 条：** knowledge 已经返回正确结果（如 TCP、RAG 基础定义），但模型仍重复调用。这不是匹配问题——是 Agent 编排问题。
...
Stage 1 剩余 4 条 rag 失败 + P2.6 全量 DS multi_tool_planning 21/35 失败。从 `ds-bench-v1.json` 和 smoke run JSON 分析全部 25 条失败：

| 类型 | 数量 | 模式 | 具体 case |
|------|------|------|-----------|
| knowledge 重复调用 | 4 | tool 已返回有效数据，模型换措辞重试 2-6 次 | GEN-KNOW-011/012/014 |
| multi-tool 不完整 | 21 | 模型调 1 个工具后停止 | MT-001/006/021/022 等 |
...
**改善：** MT-001（旅行规划）从 v1.0 FAIL→v1.1 PASS。MT-003/005/008 保持 PASS。  
